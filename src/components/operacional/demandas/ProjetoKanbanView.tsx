...
import { UserOutlined, CalendarOutlined, CheckCircleOutlined, ClockCircleOutlined, FlagOutlined } from '@ant-design/icons';
...
const ProjetoKanbanView: React.FC<ProjetoKanbanViewProps> = ({ projetos, onProjetoClick, onUpdateProjetoStatus }) => {
  ...
  return (
    <div className="flex-grow bg-white/95 backdrop-blur rounded-xl shadow-lg p-4 overflow-hidden h-full">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 h-full">
        {kanbanColumns.map(status => {
          const columnProjetos = getProjetosByStatus(status.id);
          
          return (
          <div
            key={status.id}
            className={`${status.bgColor} rounded-xl flex flex-col h-full overflow-hidden`}
            onDrop={(e) => handleDrop(e, status.id as 'NI' | 'EA' | 'C' | 'P')}
            onDragOver={handleDragOver}
          >
            {/* Column Header */}
            <div className={`${status.headerBg} px-4 py-2 flex items-center justify-between flex-shrink-0`}>
            ...
            {/* Column Content - Cards sized to fit 3 visible */}
            <div 
              className="flex-grow p-2 gap-2 overflow-y-auto kanban-scroll flex flex-col"
            >
                ...
                return (
                  <div
                    ...
                    style={{ 
                      ...
                      minHeight: 'calc((100% - 16px) / 3)',
                      maxHeight: 'calc((100% - 16px) / 3)'
                    }}
                    draggable
                  ...
                  >
                    ...
                    <div className="p-2 h-full flex flex-col overflow-hidden">
                        ...
                        {category && (
                          <Tooltip title={category.label}>
                            <Tag 
                              ...
                              className="text-[10px] font-medium px-1.5 py-0 rounded m-0 leading-tight"
                            >
                              {category.shortLabel}
                            </Tag>
                          </Tooltip>
                      ...
                      {/* Description preview - flexible */}
                      {projeto.descricao && (
                        <p className="text-[10px] text-gray-500 line-clamp-2 leading-tight flex-shrink min-h-0 overflow-hidden">
                          {projeto.descricao}
                        </p>
                      )}
